...
// Pin a wrapper to its resolved object pointer at creation time.  Only call when Ptr is guaranteed alive at
// this instant (e.g. freshly resolved under a lock).

inline void lj_object_pin(GCobject *Obj, OBJECTPTR Ptr)
{
   Ptr->pinWeak();
   Obj->set_pinned(true);
   Obj->ptr = Ptr;
