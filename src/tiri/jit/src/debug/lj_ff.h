...
#define FFDEF(name, canonical)   name,
#include "lj_ffdef.h"
   _MAX
};
...
#define FFDEF(name, canonical)   inline constexpr unsigned int FF_##name = unsigned(FastFunc::name);
#include "lj_ffdef.h"
...
inline constexpr std::array<const char *, FF__MAX> glBuiltinCallableNames = {
   nullptr,
   nullptr,
#define FFDEF(name, canonical) canonical,
#include "lj_ffdef.h"
};
...
static_assert(builtin_callable_abi_fingerprint() IS 0x57d8433576f00d37ull,
