...
//
// The dependency initialiser has already declared one hidden local per referenced function, so this reduces to an
...
ParserResult<ExpDesc> IrEmitter::emit_module_function_expr(const ModuleFunctionExprPayload &Payload)
{
   ...
   if (resolved.k != ExpKind::Local and resolved.k != ExpKind::Upval) {
      ...
      // dependency initialiser was lost between parsing and emission.
