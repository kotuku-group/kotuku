...
struct matrix_pivot {
    static int pivot(double m[Rows][Cols], unsigned row) {
        ...
        unsigned i;
        for (i = row; i < Rows; i++) {
            if((tmp = fabs(m[i][row])) > max_val and tmp != 0.0) {
                max_val = tmp;
                k = i;
            }
        }

        if (m[k][row] IS 0.0) return -1;
...
struct simul_eq {
                        ...
                        double result[Size][RightCols])
    {
        ...
        for(i = 0; i < Size; i++) {
            for(j = 0; j < Size; j++) tmp[i][j] = left[i][j];
            for(j = 0; j < RightCols; j++) tmp[i][Size + j] = right[i][j];
        }
        ...
        for(k = 0; k < Size; k++) {
            ...
            for(j = k; j < Size + RightCols; j++) tmp[k][j] /= a1;

            for(i = k + 1; i < Size; i++) {
                a1 = tmp[i][k];
                for (j = k; j < Size + RightCols; j++) tmp[i][j] -= a1 * tmp[k][j];
            }
        }
        ...
        for(k = 0; k < RightCols; k++) {
            ...
            for(m = int(Size - 1); m >= 0; m--) {
                ...
                for(j = m + 1; j < Size; j++) result[m][k] -= tmp[m][j] * result[j][k];
            }
        }
        return true;
    }
