...
For a table containing a contiguous sequence at integer keys `0` through `n - 1`, with no value at key `n`, the `#`
operator returns `n`.

A table that has ever been addressed with a *non-numeric* key is not a sequence, and `#` returns `nil` for it.  The
intent is that a table used as a record or as an associative map cannot silently report a length that describes only
...
The length of a table with gaps in its integer keys is unspecified.  The runtime selects a valid sequence boundary:
either zero when key `0` is absent, or an absent key `n` immediately following a present key `n - 1`.  It does not
guarantee that it selects the first gap.  Do not use `#` to measure sparse tables:
...
Use `table.empty()` to test whether a table contains no entries.  Use `pairs()` or another iterator when every entry in a
sparse table must be counted or processed.
