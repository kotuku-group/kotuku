// VERIFIED: 2026-02-15
// CHANGED: 2026-07-31 — Replaced ambiguous subscription guidance.
// UPDATED: 2026-08-11 - Migrated method calls in the input examples to dot syntax (commit 7e6746950)
// UPDATED: 2026-08-15 - Vector subscriptions capture the registration context; the input namespace still begins
//                       at the root (commit 1a38f18e0)
// UPDATED: 2026-08-17 - input.subscribe(), input.keyboard() and input.requestItem() capture the registration context
//                       and require a function value (commit f052ec04d)
...
* **Global subscriptions** use the `input` namespace to receive raw device events, optionally filtered by a `Surface`.  Use them to interface directly with the host display system.
...
The subscription is registered with the framework, so `Callback` captures the context that is current at the point of subscription and observes it on every event (<<sec-context-and-callbacks>>).
...
The `Callback` is captured with the context that is current at the point of subscription and observes that table on every invocation.  `input.keyboard()` and `input.requestItem()` follow the same rule.  See <<sec-context-and-callbacks>>.
