// VERIFIED: 2026-02-12
// CHANGED: 2026-07-30 — Clarified math.fmod truncated-division semantics and its divergence from the % operator.
...
Returns the floating-point remainder of the truncated division of `X` by `Y`.  The result has the same sign as `X`.
...
NOTE: `math.fmod()` and the `%` operator use different conventions and disagree whenever the operands have opposite signs.  `math.fmod()` takes the sign of `X`, whereas `%` takes the sign of the right operand.  `math.fmod(-5, 3)` is `-2`, whereas `-5 % 3` is `1`.  See <<sec-arithmetic-operators>>.
...
[.api]`result = math.clamp(Value, Lower, Upper)`

Constrains `Value` to the inclusive range [`Lower`, `Upper`] and returns the result.  Values below `Lower` are raised to `Lower` and values above `Upper` are lowered to `Upper`; values already within the range are returned unchanged.  It is functionally equivalent to `math.min(math.max(Value, Lower), Upper)`.

All three arguments are required.  If the bounds are supplied in reverse order (`Lower` greater than `Upper`), `Upper` takes precedence and is always returned.
...
|One (`High`)
|Returns an integer in the range [0, `High`), that is, from `0` up to but excluding `High`.

|Two (`Low`, `High`)
|Returns an integer in the range [`Low`, `High`], with both bounds inclusive.
|===

The single-argument form is zero-based and exclusive of its bound, so `math.random(#arr)` selects a valid index of `arr` directly.  The two-argument form is unaffected and retains inclusive bounds, so `math.random(1, N)` is the way to obtain a one-based value.

[source,tiri]
----
math.random()        -- e.g. 0.523 (float in [0, 1))
math.random(10)      -- e.g. 7     (integer in [0, 10) -- never 10)
math.random(1)       -- always 0   (the range [0, 1) holds one integer)
