// VERIFIED: 2026-08-05
// UPDATED: 2026-08-11 - String methods are called with '.' as built-in method calls (commit 7e6746950)
// UPDATED: 2026-08-14 - The array string element is now spelled 'str'; 'string' is rejected (commit 148480f9d)
// UPDATED: 2026-08-14 - Documented self referencing inside f-string interpolation, which now parses in every position
//                       within the string (commit 148480f9d)
// UPDATED: 2026-08-18 - string.byte(), string.sub() and string.find() migrated to half-open (exclusive stop) range
//                       semantics; documented the deprecated string.substr() alias (commit 3e99d69ac)
// UPDATED: 2026-08-21 - Documented substring membership via the 'in' operator; string.contains() is no longer
//                       publicly callable (commit 2b788f198)
...
Every string function that accepts a `Start` and `Stop` index pair defines a half-open span `[Start, Stop)`: the byte at `Start` is included and the byte at `Stop` is excluded.  The number of bytes selected is therefore `Stop - Start`.  A negative index is resolved against the string length before the span is evaluated, and a resolved `Stop` remains exclusive.  A span in which `Stop` is not greater than `Start` after resolution is empty.  Functions that return a position pair, such as `string.find()`, return the same half-open form, so a returned pair can be passed directly to `string.sub()`.

Most string library functions raise a type error if a required string argument is not a string.  The exceptions are `string.trim()`, `string.rtrim()`, `string.escXML()` and `string.unescapeXML()`, which treat a `nil` argument as an empty string and return `''`.  Each function's specification states its own behaviour.  Functions that accept optional parameters treat missing arguments as described in each function's specification.
...
Removes trailing whitespace from `String`.  Leading whitespace is preserved.  Whitespace characters are: space, tab, newline, and carriage return.  Returns an empty string if `String` is `nil` or contains only whitespace.
...
=== Slicing with Ranges
indexterm:[strings, slicing] indexterm:[ranges, string slicing] Strings support range objects as indices for substring extraction.  Indexing is zero-based.  The range must be whole-numbered as defined in <<sec-range-index-requirement>>; a fractional range raises an error.
