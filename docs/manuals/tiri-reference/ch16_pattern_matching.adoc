...
The empty table pattern `{}` is a type test, not an emptiness test.  It matches *any* table value, including a populated one, and does not match non-table values (`nil`, strings, numbers, booleans):

CAUTION: This is the opposite of the meaning `{}` carries under extended falsiness, where an empty table is falsey and a populated one is truthy (<<sec-truthiness>>).  In a pattern, `{}` reads as "is a table"; use a guard such as `{} when table.empty(value)` to test for emptiness.
