// VERIFIED: 2026-02-12
// UPDATED: 2026-07-26 - Migrated examples to range loops following removal of the Lua-style numeric for syntax (commit 1f23b978)
// CHANGED: 2026-07-29 — Added <<sec-finalisers-and-scope-cleanup>> to distinguish nondeterministic __gc finalisation from deterministic <close> and defer (commit 279cb0a7)
// CHANGED: 2026-07-31 — Tightened deferred-parameter guidance.
// UPDATED: 2026-08-06 - Removed 'boolean' from the deferred-expression type names, which now rejects it (commit 66ad8a7dc)
// UPDATED: 2026-08-11 - Migrated method calls in the resource examples to dot syntax (commit 7e6746950)
// UPDATED: 2026-08-11 - Documented the current context of a __close handler (commit 798665547)
// UPDATED: 2026-08-13 - The __close example now designates its receiver as an entity; noted context blocks (commit d3638efc0)
// UPDATED: 2026-08-16 - Migrated the __close examples to the `metamethod` form, which is now the standard for metatable
//                       callables; added the Handler Context subsection covering the one case that still requires
//                       `function` (commit 58e7bad8a)
...
Deferred blocks do not execute during error unwinding caused by `error()` or runtime exceptions.  For error-safe cleanup, use to-be-closed variables (<<sec-to-be-closed-variables>>).
...
The two declaration forms observe different contexts, and the choice determines what self referencing reaches in the body.  The distinction carries more weight for `pass:[__]close` than for other metamethods, because the handler is commonly written inline at the point the variable is declared, where the surrounding receiver is in scope:

- A `metamethod` literal installs the value being closed as its context, as every metamethod literal does (<<sec-context-aware-metamethods>>).  The body reaches the closed value through self referencing.
- A `function` handler installs no context of its own, so it runs in the current context of the activation that declares the variable (<<sec-the-current-context>>), on both the normal and the error path.  A handler declared inside a method therefore reaches that method's receiver, and a handler declared inside a context block observes that block's activation (<<sec-context-blocks>>).

Write the handler with `metamethod` when it acts on the closed value, which is the ordinary case.  Write it with `function` only when it acts on the surrounding receiver:
...
function session.run()
   res <close> = setmetatable({}, {
      __close = function()      -- `function`, not `metamethod`
         &open = false          -- clears the field on 'session'
      end
   })
end
----
...
do
   a <close> = setmetatable({}, {
      __close = metamethod() print('close a') end
   })
   b <close> = setmetatable({}, {
      __close = metamethod() print('close b') end
   })
end
-- Output:
-- close b
...
do
   ...
   local obj <close> = setmetatable({}, {
      __close = metamethod() print('close') end
   })
end
-- Output:
-- close
...
[source,tiri]
----
mt = { __close = metamethod() print('closed') end }
...
mt = {
   __close = metamethod(Error)
      if Error then
         print('Error during close: ' .. tostring(Error))
      end
   end
}
...
[source,tiri]
----
mt1 = { __close = metamethod() error('close error') end }
mt2 = { __close = metamethod() print('still runs') end }
...
The type name is drawn from the native vocabulary and accepts every name listed in <<sec-supported-type-names>>, with the exception of the constrained forms `array<element>` and `struct<Name>`: the deferred form takes a bare name, so `array` and `struct` impose no element or layout constraint.  Any other name raises a parse-time error.
...
Returns `true` if `Value` is an unresolved deferred expression, `false` otherwise.  Because `type()` reports the declared or inferred type of a thunk rather than identifying it as one (see <<sec-type-inspection>>), `isthunk()` is the only way to distinguish a deferred expression from an ordinary value of the same type.
...
do
   local resource <close> = setmetatable(
      { file = io.open('temp:data.txt', 'r') }, handle_mt
   )
   process(resource.file.read('*a'))
end
----
