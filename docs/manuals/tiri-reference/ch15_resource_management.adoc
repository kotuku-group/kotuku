...
// UPDATED: 2026-08-19 - Reverted the __close examples to the ordinary `function` form and rewrote Handler Context:
//                       dispatch always installs the closed value as context (commit 81aa5f3)
...
function transfer(SourcePath:str, DestPath:str)
   source = obj<File> { path = SourcePath, flags = 'READ' }
   defer
      source.acFree()
   end
   dest = obj<File> { path = DestPath, flags = 'WRITE|NEW' }
   defer
      dest.acFree()
   end
