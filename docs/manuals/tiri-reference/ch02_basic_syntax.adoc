// VERIFIED: 2026-08-01
// UPDATED: 2026-08-11 - Documented the indentation rule that separates a leading-dot continuation from a
//                       current-context statement (commit 798665547)
// UPDATED: 2026-08-13 - Replaced the leading-dot rule with the line rule for self referencing ('&name'), which is
//                       what distinguishes it from the binary '&' (commit d3638efc0)
// UPDATED: 2026-08-15 - Added the reserved word `method`, and `using`, which the keyword table had omitted
// UPDATED: 2026-08-16 - Renamed the reserved word `method` to `metamethod`; `method` is now an ordinary identifier
//                       (commit 5c92d652a)
//                       (commits ac4f30a67, d3638efc0)
...
[[sec-comments]]
=== Comments
indexterm:[comments] The lexer discards all comment text.  Comments have no effect on program behaviour.  Tiri supports two comment forms.
...
[cols="6*^"]
|===
|`and`      |`as`      |`break`   |`check`    |`choose`  |`continue`
|`defer`    |`do`      |`else`    |`elseif`   |`end`     |`enum`
|`except`   |`extern`  |`false`   |`for`      |`from`    |`function`
|`global`   |`has`     |`if`      |`import`   |`in`      |`is`
|`local`    |`metamethod`|`namespace`|`nil`  |`not`     |`or`
...
A reserved word remains available as a table member name, so `t.using` and `t.with` are valid field accesses.  `metamethod` is accepted wherever an identifier is expected, and is reserved only against naming a callable and against the call position (<<sec-metamethod-literals>>).
...
Statements are separated by line breaks or semicolons.  Semicolons are optional except where a statement boundary is otherwise ambiguous.
...
[[sec-statement-ambiguity]]
==== Statement Ambiguity

A line break does not terminate a statement.  A statement that begins with `(` is therefore parsed as a continuation of the preceding expression, forming a function call:

[source,tiri]
----
a = f
(g).x = 1     -- parsed as 'a = f(g).x = 1', which is a syntax error
----

Terminate the preceding statement with a semicolon to force a statement boundary:

[source,tiri]
----
a = f;
(g).x = 1     -- two statements
----

A self reference (<<sec-self-referencing>>) is exempt from the continuation rule when it opens a line: a `&` that begins a line and is followed immediately by an identifier terminates the preceding statement.  On the same line the character reads as the bitwise AND operator, so two such statements written on one line require a semicolon between them:

[source,tiri]
----
&total = Value
&count++            -- a second statement

&reset(); &count++  -- one line, so the semicolon is required
----
...
indexterm:[numeric literals] indexterm:[binary integer literals] Numeric literals represent integer and floating-point values.
...
Decimal integers are sequences of digits.  Hexadecimal integers begin with `0x` or `0X` followed by hexadecimal digits (`0`-`9`, `a`-`f`, `A`-`F`).  Binary integers begin with `0b` or `0B` followed by binary digits (`0` or `1`).
...
If the pattern is syntactically invalid, compiling the regex object raises an exception at runtime.
