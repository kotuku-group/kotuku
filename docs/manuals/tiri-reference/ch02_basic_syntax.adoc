...
|`local`    |`method`  |`namespace`|`nil`    |`not`     |`or`
...
A reserved word remains available as a table member name, so `t.using` and `t.with` are valid field accesses.  `method` is accepted wherever an identifier is expected, and is reserved only against naming a callable and against the call position (<<sec-context-first-methods>>).
