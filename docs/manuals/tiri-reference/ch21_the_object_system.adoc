// VERIFIED: 2026-02-13
// CHANGED: 2026-07-27 — Added Result Types under Actions & Methods; native action and method results now carry static types (commit 8905c1966).
...
// CHANGED: 2026-07-31 — Standardised exception terminology and present-tense behaviour.
...
// UPDATED: 2026-08-15 - new() and _state() are canonical built-in methods dispatched with an explicit receiver; added
//                       the sec-object-method-dispatch section (commit d26d1d4b9)
// UPDATED: 2026-08-15 - Callbacks passed as method arguments or assigned to fields capture the registration context;
//                       object.subscribe() still begins at the root (commit 1a38f18e0)
// UPDATED: 2026-08-17 - object.subscribe() captures the subscription context and requires a function value; rewrote
//                       the Subscription Context section (commit f052ec04d)
// UPDATED: 2026-08-20 - Noted that array-field reads copy, and that a <view> local aliases native field storage
//                       (commit ea9fbc23c)
// UPDATED: 2026-08-22 - Added obj<Class> typed constructor as preferred form; repositioned obj.new() as fallback
//                       (commit cf5728552)
...
file = obj.new('file', { path = 'readme.md', flags = FL_READ })
----
...
Reads a field value by name.  If the object is inaccessible or the field cannot be read, returns `Default` or `nil`.  The function never raises an exception; `Default` has priority.
...
Writes a field value by name and returns an `ERR` code.  It is equivalent to `object.fieldName = value`, but returns the error code rather than raising an exception on failure.  Calling it inside a `try` statement promotes error codes to exceptions.
...
`setKey()` raises exceptions when called inside a `try` statement.
...
[source,tiri]
----
file = obj<File> { path = 'readme.md', flags = '!READ' }
-- ...
