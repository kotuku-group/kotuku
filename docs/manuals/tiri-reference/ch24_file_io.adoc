...
=== File Handle Methods
indexterm:[file system, handles] File handles returned by `io.open()` and related functions support the following methods, called with the colon syntax (e.g. `file:read()`).  Operating on a closed file handle raises an exception.
