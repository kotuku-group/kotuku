// VERIFIED: 2026-08-22
// UPDATED: 2026-09-01 - mtNext() returns ERR_EndOfSequence; ERR_DirEmpty was renamed (commit 6d224df2f)
...
Filesystem failures in the `io` library raise exceptions.  This differs from standard Lua, which returns `nil` plus an error string.  Use `try-except` blocks (<<ch-error-handling>>) to handle errors.
...
Opens the file at `Path`, requesting access permissions specified by `Mode`.  If `Mode` is not set, the file is opened in read-only mode.  Raises an exception if the file cannot be opened.
...
Writes each argument to the default output file.  The default output file is initially `stdout` but can be changed with `io.output()`.  Arguments must be strings or numbers.  Raises an exception if a write error occurs.
...
Reads the entire contents of the file at `Path` and returns it as a string.  Raises an exception if the file cannot be opened or read.  Returns an empty string if the file is empty.
...
Writes `Content` to the file at `Path`, creating the file if it does not exist or overwriting it if it does.  Raises an exception if the file cannot be created or written to.
...
=== File Handle Methods
indexterm:[file system, handles] File handles returned by `io.open()` and related functions support the following methods, called with dot notation on the handle (e.g. `file.read()`).  The handle is passed as the receiver automatically; see <<sec-built-in-method-calls>>.  Operating on a closed file handle raises an exception.
...
Writes each argument to the file.  Arguments must be strings or numbers.  Returns the file handle to allow chained calls.  Raises an exception if a write error occurs.
...
Returns an `ERR` code.  If the user cancels the operation, `ERR_Cancelled` is returned.  Errors are presented to the user with a dialog box and are not raised as exceptions.
...
File objects are created with `obj<File> { Options }`.  Refer to <<ch-the-object-system>> for general object creation patterns.
...
fl = obj<File> { path='temp:data.txt', flags=FL_READ }

-- Create a new file for writing
fl = obj<File> { path='temp:output.txt', flags=FL_NEW|FL_WRITE }

-- Open a folder for enumeration
folder = obj<File> { src='system:scripts/' }
----
...
|`mtNext()`
|When the object represents a folder, returns the next file in the folder index as a new File object.  Returns `err, file` where `err` is `ERR_EndOfSequence` when no more entries remain.
...
Calling `raise ERR_Terminate` in the callback cancels the watch.
