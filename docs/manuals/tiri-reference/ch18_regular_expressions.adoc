...
indexterm:[regex, creation] Compiles `Pattern` into a reusable regex object.  `Flags` is an optional bitmask of compilation flags (see <<sec-flags-and-options>>).  If `Pattern` is syntactically invalid, the function raises an exception with error code `ERR_Syntax` and a descriptive message.
...
If the pattern is syntactically invalid, the implicit `regex.new()` call raises an exception.  Wrap patterns from external sources in a `try` block (see <<ch-error-handling>>).
...
|`error`
|string or nil
|Always `nil` for regex objects accessible to scripts.  Invalid patterns raise an exception during construction rather than producing an object with an error state.
|===
