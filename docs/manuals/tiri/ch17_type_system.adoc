// VERIFIED: 2026-02-12
// CHANGED: 2026-07-27 — Added sec-runtime-type-contracts; documented userdata annotations, exact contract identity, and the advisory status of inferred globals.
...
An inferred type is enforced at runtime for locals and for upvalues captured from typed locals.  An inferred type on an unannotated global constrains compile-time diagnostics only; a value delivered through an `any` expression is not rejected at runtime.  See <<sec-runtime-type-contracts>>.
