...
The `by` clause enforces two constraints at runtime: the step must be a non-zero integer.  Violating either raises an error.
...
`tostring()` produces a brace-delimited representation using the `to`/`into` keywords.  A `by` clause is included when the step differs from the inferred default:
...
indexterm:[ranges, iteration] Ranges integrate with Tiri's generic `for` loop.  When a range literal appears directly in the loop header, the parser optimises it to a numeric for loop when all operands are constants.
