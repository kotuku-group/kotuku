...
indexterm:[enum declarations] indexterm:[enumerations] indexterm:[constants, enum] An enum declaration generates a set of named integer constants from a compact definition.  The `enum` keyword is contextual -- it is not reserved and remains usable as an ordinary identifier outside enum declaration syntax.

==== Syntax

[listing]
....
enum_declaration = prefix "=" "enum" "{" member_list "}" ;
...
The `prefix` and each member name must use uppercase identifier style: only uppercase letters, digits, and underscores, with at least one letter.  The prefix determines the generated constant names by joining `PREFIX_MEMBER` with an underscore separator.

==== Declaration Forms

Enum declarations support three scoping forms: bare, `local`, and `global`.

[source,tiri]
----
-- Bare declaration (generates local constants)
STATUS = enum {
   IDLE,
   RUNNING,
   DONE,
...
The bare and `local` forms generate local `<const>` bindings.  The `global` form generates global `<const>` bindings.

==== Generated Constants

Each member produces a constant named `PREFIX_MEMBER` with an integer value.  The first member defaults to `0`.  Each subsequent member without an explicit value receives the previous member's value plus one.
...
-- Generates:
-- local DIRECTION_NORTH <const>:num = 0
-- local DIRECTION_EAST  <const>:num = 1
-- local DIRECTION_SOUTH <const>:num = 2
-- local DIRECTION_WEST  <const>:num = 3
----

The prefix identifier itself does not create a runtime binding.  Only the generated `PREFIX_MEMBER` constants exist at runtime.

==== Explicit Values

Members can specify explicit integer values.  Subsequent members without an explicit value auto-increment from the last explicit value.
...
Negative values, positive signs, and hexadecimal literals are accepted:
...
Non-integer values (e.g. `1.5`) are rejected at parse time.

==== Immutability

Generated constants are `<const>` bindings.  Reassignment is a compile-time error:

[source,tiri]
----
local MODE = enum { ON, OFF }

MODE_ON = 5   -- error: cannot assign to const local 'MODE_ON'
----

==== Contextual Keyword

The identifier `enum` is not reserved.  Outside of enum declaration syntax it behaves as an ordinary variable name:
...
The parser recognises an enum declaration only when an uppercase identifier is followed by `= enum {`.
